...
def glob(includes, excludes=[], build_env=None):
  ...
  for root, dirs, files in symlink_aware_walk(search_base):
    ...
    # expect a leading './'
    if relative_root == '.' or relative_root == '':
...
class BuildFileProcessor:
  ...
  def process(self, build_file):
    ...
    if relative_path_to_build_file.startswith(self.project_root.replace('\\', '/')):
      base_path = relative_path_to_build_file[len(self.project_root) + 1:self.len_suffix]
    else:
...
def main():
  ...
  if args:
    ...
    ignore_paths = [p.replace('\\', '/') for p in options.ignore_paths]
    build_files = []

    # The project root path plus the trailing slash.
    project_root_prefix = len(os.path.abspath(project_root)) + 1
    for dirpath, dirnames, filenames in symlink_aware_walk(project_root):
      ...
      dirnames[:] = [d for d in dirnames if not ((dirpath + '/' + d)[project_root_prefix:] in ignore_paths)]
